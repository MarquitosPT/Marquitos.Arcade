...
- `src/MarquitosArcade/Components/Pages/Privacidade.razor`: política de privacidade em `/privacidade`, ligada a partir da nota no fim da home, a seguir ao catálogo (`.home-legal` em `Home.razor`) — e não do rodapé, que é uma barra de uma linha onde o link partia a linha em ecrãs estreitos. É a página apontada como política de privacidade do site e da app, por isso o URL é para manter estável. O texto vive no próprio `.razor` (não há CMS nem markdown por trás) e usa o mesmo painel de vidro das páginas simples (`.page-shell`), com a tipografia longa na secção `.legal` do `styles.css`. Ao alterá-la, actualizar as duas datas da página — a do topo e a da linha de versão no fim.
