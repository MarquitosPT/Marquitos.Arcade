// Carrossel de páginas de cartões.
//
// Quantos cartões cabem numa página não se decide aqui: lê-se do CSS, nas
// variáveis `--page-cols` e `--page-rows` do elemento (ver css/carousel.css).
// Quem sabe o tamanho do ecrã é o CSS, e assim há um sítio só a dizer que num
// telemóvel ao alto cabem quatro e num computador cabem seis — em vez de uma
// regra no CSS e outra, a repeti-la, em JavaScript.
//
...
// ponteiro, e não com o scroll horizontal do browser: o ecrã onde isto vive
// trava o gesto lateral (`touch-action: pan-y`, ver css/screens.css) para o
// arrasto não levar o documento atrás, e essa trava alcançaria também um
// scroll nativo cá dentro.
 ...
 * @param {string} [options.pageClass='levelPage'] Classe de cada página.
 * @param {(page: number, pageCount: number) => void} [options.onPageChange]
 */
export function createCarousel({ root, viewport, track, prev, next, dots }, { pageClass = 'levelPage', onPageChange = null } = {}) {
     ...
     * nível que estava escolhido não pode desaparecer.
     */
     ...
     * quase sempre um cartão. Sem isto, virar a página começava o nível que
