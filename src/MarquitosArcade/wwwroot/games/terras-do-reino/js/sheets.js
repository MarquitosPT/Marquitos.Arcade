...
import {
    BUILDING, BUILDINGS, CLEAR_COST, DEMOLISH_REFUND, FLATTEN_COST, FLATTEN_STONE, HAPPY_LEISURE, NEAR_FEATURES, RESOURCE, ROAD_COST, SERVICE_EFFECTS,
    TOWNS
} from './config.js';
import {
...
function recipeText(def) {
    ...
    if (def.service) return `Serve até ${def.service.residents} moradores: ${serviceEffect(def)}`;
...
/** O efeito de um serviço, com todo o povo servido: "até +20% de impostos". */
function serviceEffect(def) {
    return Object.entries(SERVICE_EFFECTS).filter(([k]) => def.service[k])
        .map(([k, e]) => `até +${pct(def.service[k])} ${e.text}`).join(', ');
}
...
function castleView() {
    ...
    return {
        ...
        html: `<div class="sheetSection"><div class="statGrid">
            ...
            <div class="sheetSection">${servicesHtml()}</div>
...
/** Os serviços do reino: que parte do povo cada um serve agora, e o efeito que isso dá. */
function servicesHtml() {
    const cared = game.derived.cared || {};
    const rows = BUILDINGS.filter((def) => def.service).map((def) => {
        if (def.tier > castleInfo().tier) return ['warn', `🔒 ${def.emoji} ${def.name}: castelo nível ${def.tier} (${serviceEffect(def)})`];
        const ratio = cared[def.id] || 0;
        return [ratio >= 0.995 ? 'ok' : 'warn', `${def.emoji} ${def.name}: ${pct(ratio)} do povo servido (${serviceEffect(def)})`];
    });
    return `<p class="sheetText">Os serviços do reino servem o povo todos os dias, enquanto tiverem gente a trabalhar:
        a 🏫 escola sobe os impostos, o 🏥 centro de saúde apressa as oficinas e o 📮 posto de correios melhora os preços do mercado.</p>
        ${rows.map(([tone, text]) => `<div class="statusLine ${tone}" style="margin-top:6px">${text}</div>`).join('')}`;
}
...
/** Um serviço a funcionar: a barra é a parte do povo que serve. */
const SERVICE_OK = ['ok', '✅ A funcionar — a barra é a parte do povo servida'];
...
function tileView(arg) {
    ...
    if (b && b.owner === 'player') {
        ...
        } else if (def.recipe || def.plants || def.farms || def.serves || def.lodges || def.service) {
            const venue = def.lodges ? { ok: INN_OK, noInput: INN_HUNGRY }
                : def.service ? { ok: SERVICE_OK }
                    : def.serves && !def.recipe ? { ok: VENUE_OK } : {};
